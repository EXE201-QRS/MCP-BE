...
import { ServicePlanType } from '@/routes/service-plan/service-plan.model'
import { SubscriptionType } from '@/routes/subscription/subscription.model'
import { BadRequestException, Injectable, Logger } from '@nestjs/common'
import PayOS from '@payos/node'
import { mapEnumToDays } from '../helpers'
...
export class PayOSService {
   ...
   */
  generatePaymentUrls(paymentId: number) {
    const baseUrl = process.env.APP_URL || 'http://localhost:4000'

    return {
      returnUrl: `${baseUrl}/payments/payos/return?paymentId=${paymentId}`,
      cancelUrl: `${baseUrl}/payments/payos/cancel?paymentId=${paymentId}`
    }
  }
  ...
  createPayOSItems({
    ...
    const count = parseInt(mapEnumToDays(subs.durationDays)) / 30
    //quantiy: = số duration của tháng / 30
    return [
      {
        name: ser.name,
        quantity: count,
